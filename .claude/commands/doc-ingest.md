...
- PDF → ne pas l'ingérer tel quel : le convertir d'abord en markdown propre
  (OCR externe), déposer le résultat dans `$VAULT/inbox/`, puis reprendre ce
  circuit normalement.
...
3. Mettre à jour `$VAULT/INDEX.md` : ajouter chaque nouvelle note dans sa
   section, sous forme `- [[<nom-de-fichier-sans-extension>]] — <description en
   quelques mots>`.
